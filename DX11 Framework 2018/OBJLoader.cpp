...
MeshData* OBJLoader::Load(char* filename, ID3D11Device* _pd3dDevice, BreaksEngine* audioEngine, bool invertTexCoords)
{
	...
	if (!binaryInFile.good())
	{
		...
		else
		{
			...
			meshData->fmodVerts = meshVertices;
			FMOD_VECTOR* finalVert = new FMOD_VECTOR[meshData->fmodVerts.size()];

			for (size_t i = 0; i < meshData->fmodVerts.size(); i++)
			{
				finalVert[i] = { meshData->fmodVerts[i].x,  meshData->fmodVerts[i].y, meshData->fmodVerts[i].z };
			}
	...
	else
	{
		...
		//meshData->fmodVerts = finalVerts;
		FMOD_VECTOR* finalVert = new FMOD_VECTOR[numVertices];
